...
namespace TgBot.Tasks
{
    public class CreateTaskDialog:FormDialog
    {
        ...
        public CreateTaskDialog():base(null,null)
        {

        }
        public CreateTaskDialog(ChatId chatId, User user,
        ...
        protected override async Task<DialogResult> OnCompleteAsync(ITelegramBotClient bot, CancellationToken cancellationToken)
        {
            ...
            await TaskBot.NotifyGroups(bot, $"{service.GetUserProfile(this.from.Id.ToString()).Name()} created task {TaskBot.TaskLink(task.Id,task.CodeName)}",cancellationToken);
            return DialogResult.Terminated;
        }
    }
}
